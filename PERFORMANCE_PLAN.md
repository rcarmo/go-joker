...
- Typed IR v2 now runs automatically for eligible primitive/string loops unless disabled with `JOKER_IR_TYPED=off`, using cached IR analysis and a tagged value stack. It now supports generic string `nth`, typed string-builder slots, and experimental string-int map values behind `JOKER_IR_TYPED_MAP=1`.
