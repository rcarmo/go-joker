...
- Typed IR v2 has started behind `JOKER_IR_TYPED=1`, with a tagged primitive/string executor for eligible string loops.
...
- Typed IR v2 prototype (`JOKER_IR_TYPED=1`) now runs primitive/string-only loops with tagged values, reducing allocations in string append and reverse-complement probes while staying off by default.
