...
runtime-contract-check:
	$(GO) test ./core -run 'TestIRExecutionMetadata|TestEscapeAnalysis|TestIRMakeFn|TestRuntimeExecutionAdapter|TestIRCompileFailure|TestNativeHelperEligibility|TestChannelCloseIsIdempotentUnderConcurrency|TestWasmRawInt|TestWasmExecRawIntegerResultUsesNativeRange' -count=$(TEST_COUNT) -timeout=120s

std-contract-check:
	$(GO) test ./std/http ./std/io ./std/strconv ./std/time ./std/markdown ./std/os ./std/system ./std/runtime -count=$(TEST_COUNT) -timeout=120s
