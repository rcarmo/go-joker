...
func TestExecutorFilesUseRuntimeExecutionAdapterForProgramState(t *testing.T) {
	for _, file := range []string{
		...
		for lineNo, line := range strings.Split(string(data), "\n") {
			...
			if lineText == "case *coretypes.StringCursor:" || lineText == "return (*coretypes.StringCursor)(v.p)" {
				continue
			}
			if strings.Contains(line, "*coretypes.StringCursor") || strings.Contains(line, ".Char()") || strings.Contains(line, ".Next()") || strings.Contains(line, ".Done()") {
...
func TestRuntimeExecutionAdapterStringOps(t *testing.T) {
	...
	cur := coretypes.NewStringCursor("x")
...
func TestCharToStringFast(t *testing.T) {
	...
	if got := coretypes.CharToStringObjectFast('é'); got.(coretypes.String).S != "é" {
...
// Native integer dispatch must preserve language-level arity errors.
...
func TestAuditIRExactDivision(t *testing.T) {
	fn := evalTestScript(t, `(fn [x y] (/ x y))`).(*Fn)
	prog := irCompileFn(fn)
	if prog == nil {
		t.Fatal("IR compilation failed")
	...
	for _, pair := range [][2]int{{1, 2}, {4, 2}, {-1, 2}, {1, 0}} {
		...
		want := capture(func() coretypes.Object { return procDivide(args) })
		for name, run := range map[string]func() coretypes.Object{
			"boxed":  func() coretypes.Object { return irExec(prog, args) },
			"typed":  func() coretypes.Object { return irExecTyped(prog, args) },
			"nanbox": func() coretypes.Object { return irExecTypedNB(prog, args) },
			"inline": func() coretypes.Object {
				slots := make([]irValue, runtimeExec.ProgramNumSlots(prog))
				for i, v := range args {
			...
			t.Run(fmt.Sprint(name, pair), func(t *testing.T) {
				got := capture(run)
				if got != want {
