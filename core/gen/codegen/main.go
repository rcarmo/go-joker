...
const masterCodeFilename = "a_generated_bootstrap_payloads.go"
const masterDataFilename = "a_data.go"
const generatedDataFilenamePattern = "generated/a_%s_data.go"
const generatedLinterPayloadsFilename = "generated/linter_payloads_gen.go"
...
func linterDataVarName(filename string) string {
	name := filename[0 : len(filename)-5] // assumes .joke extension
	return "Linter" + strings.Join(func() []string {
...
func main() {
	...
	// Emit per-namespace bootstrap snippets into the grouped master file.
	var namespaceBootstrap []string
...
`
		...
		if VerbosityLevel > 0 {
			fmt.Printf("OUTPUTTING %s into %s (mappings=%d)\n", nsName, masterCodeFilename, len(ns.Mappings()))
		}
...
`[1:],
		...
		namespaceBootstrap = append(namespaceBootstrap, fileContent)
	}

	/* Output the grouped master file. */
...
%s
	...
	groupedRuntime := strings.TrimSpace(strings.Join(namespaceBootstrap, "\n") + "\n" + r)
	var tr = [][2]string{
		{"{imports}", imp},
...
`[1:]
	...
	generateLinterPayloadRegistry(linterFiles)

	/* Generate source manifest used by root core bootstrap. */
	generateBootstrapContractFile()
}
...
func coreTypeString(s string) string {
	...
	for _, moved := range []string{"Int", "Double", "Boolean", "Char", "Time", "Regex", "BigInt", "BigFloat", "Ratio", "Comment", "String"} {
		if s == moved {
