...
func (r *Record) EntryAt(key Object) *ArrayVector {
	if ok, v := r.Get(key); ok {
		av := EmptyArrayVector().Conj(key).(*ArrayVector).Conj(v).(*ArrayVector)
		return av
	}
	return nil
}
...
func (r *Record) Seq() Seq {
	...
	for i, fname := range r.rtype.fields {
		entries = append(entries, NewVectorFrom(MakeKeyword(fname), r.bases[i]))
	}
	if r.ext != nil {
		for iter := r.ext.Iter(); iter.HasNext(); {
			p := iter.Next()
			entries = append(entries, NewVectorFrom(p.Key, p.Value))
		}
	}
