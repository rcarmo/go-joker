...
func TestExecutorFilesUseRuntimeExecutionAdapterForProgramState(t *testing.T) {
	for _, file := range []string{
		...
		for lineNo, line := range strings.Split(string(data), "\n") {
			...
			if strings.Contains(line, "Seqable") || strings.Contains(line, "Conjable") || strings.Contains(line, "Counted") || strings.Contains(line, "Associative") || strings.Contains(line, "*TransientVector") || strings.Contains(line, "*ArrayVector") || strings.Contains(line, "&ArrayVector") {
...
func TestTransientMapStringKeys(t *testing.T) {
	...
	tm.AssocInPlace(String{S: "alpha"}, coretypes.Int{I: 1})
	tm.AssocInPlace(String{S: "beta"}, coretypes.Int{I: 2})
	tm.AssocInPlace(String{S: "alpha"}, coretypes.Int{I: 3})
	if tm.Count() != 2 {
...
func TestTransientMapProcs(t *testing.T) {
	...
	if procConjBang([]Object{tm, String{S: "b"}, coretypes.Int{I: 2}}) != tm {
	...
	if ok, got := persisted.Get(String{S: "b"}); !ok || !got.Equals(coretypes.Int{I: 2}) {
...
func TestIRTypedUnicodeCount(t *testing.T) {
  ...
  (if (= i 2)
	...
	requireInt(t, irExecTyped(prog, []Object{coretypes.Int{I: 0}, String{S: "é"}}), 3)
}
...
func TestIRTypedGenericStringNth(t *testing.T) {
  ...
  (if (= i 4)
	...
	got := irExecTyped(prog, []Object{coretypes.Int{I: 0}, String{S: "ACGT"}, String{S: ""}})
...
func TestIRTypedStringLoop(t *testing.T) {
  ...
  (loop [i 0 s ""]
	...
	got := irExecTyped(prog, []Object{coretypes.Int{I: 0}, String{S: ""}})
	requireInt(t, got, 4)
}
