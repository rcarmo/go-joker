...
func installConcurrencyExt() {
	...
	installMacro(ns, "future", func(args []Object) Object {
		// args: &form, &env, body...
		body := args[2:]
		fnForm := NewListFrom(append([]Object{MakeSymbol("fn"), NewVectorFrom()}, body...)...)
...
func procAlts(args []Object) Object {
	...
	if info.isPut {
		// Put completed.
		return NewVectorFrom(MakeBoolean(true), info.ch)
	}
	// Take completed.
	if !recvOK {
		// Channel closed.
		return NewVectorFrom(NIL, info.ch)
	}
	fr := recv.Interface().(FutureResult)
	if fr.err != nil {
		panic(fr.err)
	}
	return NewVectorFrom(fr.value, info.ch)
}
