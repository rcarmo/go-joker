...
func (g *GoroutineState) Current(id int64) any {
	g.mu.RLock()
	if g.spawned.Load() == 0 || id == g.mainID {
		v := g.mainState
		g.mu.RUnlock()
		return v
	}
...
func (g *GoroutineState) HasSpawned() bool {
	return g.spawned.Load() != 0
}
...
func (p *GoRTPool) Current() any {
	if !p.state.HasSpawned() {
		return p.state.mainState
	}
	return p.state.Current(p.goid())
}
