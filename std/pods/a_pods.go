...
func installPodsNamespace(ns *Namespace) {
	...
	ns.InternVar("load-pod", loadPod_, MakeMeta(NewListFrom(NewVectorFrom(coretypes.MakeSymbol(STRINGS.Intern, "path-or-name")), NewVectorFrom(coretypes.MakeSymbol(STRINGS.Intern, "path-or-name"), coretypes.MakeSymbol(STRINGS.Intern, "version-or-args"))), "Starts a Babashka pod process, sends describe, registers it, and returns its pod id.", "1.0"))
	ns.InternVar("invoke", invokePod_, MakeMeta(NewListFrom(NewVectorFrom(coretypes.MakeSymbol(STRINGS.Intern, "pod-id"), coretypes.MakeSymbol(STRINGS.Intern, "var"), coretypes.MakeSymbol(STRINGS.Intern, "args")), NewVectorFrom(coretypes.MakeSymbol(STRINGS.Intern, "pod-id"), coretypes.MakeSymbol(STRINGS.Intern, "var"), coretypes.MakeSymbol(STRINGS.Intern, "args"), coretypes.MakeSymbol(STRINGS.Intern, "opts"))), "Synchronously invokes a pod var and returns its decoded result. Supports JSON, EDN, and Transit+JSON pod payloads; opts may include :timeout-ms.", "1.0"))
	ns.InternVar("bencode-encode", bencodeEncode_, MakeMeta(NewListFrom(NewVectorFrom(coretypes.MakeSymbol(STRINGS.Intern, "x"))), "Encodes a Joker value as bencode bytes, returned as a string.", "1.0"))
	ns.InternVar("bencode-decode", bencodeDecode_, MakeMeta(NewListFrom(NewVectorFrom(coretypes.MakeSymbol(STRINGS.Intern, "s"))), "Decodes a bencode string into Joker data.", "1.0"))
}
