...
func installPodsNamespace(ns *Namespace) {
	...
	ns.InternVar("invoke", invokePod_, MakeMeta(NewListFrom(NewVectorFrom(MakeSymbol("pod-id"), MakeSymbol("var"), MakeSymbol("args")), NewVectorFrom(MakeSymbol("pod-id"), MakeSymbol("var"), MakeSymbol("args"), MakeSymbol("opts"))), "Synchronously invokes a pod var and returns its decoded result. JSON payloads are supported initially.", "1.0"))
