...
func TestPodTransitPayloadSupport(t *testing.T) {
	...
	encoded, err := p.encodeArgs([]coretypes.Object{coretypes.MakeKeyword(STRINGS.Intern, "k")})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPodEDNPayloadSupport(t *testing.T) {
	...
	encoded, err := p.encodeArgs([]coretypes.Object{coretypes.MakeString("x"), coretypes.MakeKeyword(STRINGS.Intern, "k")})
	if err != nil {
		t.Fatal(err)
	}
	if encoded != "[\"x\" :k]" {
