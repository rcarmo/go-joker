// This file is generated by generate-std.joke script. Do not edit manually!

package url

import (
	. "github.com/rcarmo/go-joker/core"
	coretypes "github.com/rcarmo/go-joker/core/types"
	"net/url"
)

var __parse_query__P ProcFn = __parse_query_
var parse_query_ Proc = Proc{Fn: __parse_query__P, Name: "parse_query_", Package: "std/url"}

func __parse_query_(_args []coretypes.Object) coretypes.Object {
	_c := len(_args)
	switch {
	case _c == 1:
		s := coretypes.ExtractString(_args, 0)
		_res := parseQuery(s)
		return _res

	default:
		PanicArity(_c)
	}
	return NIL
}

var __path_escape__P ProcFn = __path_escape_
var path_escape_ Proc = Proc{Fn: __path_escape__P, Name: "path_escape_", Package: "std/url"}

func __path_escape_(_args []coretypes.Object) coretypes.Object {
	_c := len(_args)
	switch {
	case _c == 1:
		s := coretypes.ExtractString(_args, 0)
		_res := url.PathEscape(s)
		return coretypes.MakeString(_res)

	default:
		PanicArity(_c)
	}
	return NIL
}

var __path_unescape__P ProcFn = __path_unescape_
var path_unescape_ Proc = Proc{Fn: __path_unescape__P, Name: "path_unescape_", Package: "std/url"}

func __path_unescape_(_args []coretypes.Object) coretypes.Object {
	_c := len(_args)
	switch {
	case _c == 1:
		s := coretypes.ExtractString(_args, 0)
		_res := pathUnescape(s)
		return coretypes.MakeString(_res)

	default:
		PanicArity(_c)
	}
	return NIL
}

var __query_escape__P ProcFn = __query_escape_
var query_escape_ Proc = Proc{Fn: __query_escape__P, Name: "query_escape_", Package: "std/url"}

func __query_escape_(_args []coretypes.Object) coretypes.Object {
	_c := len(_args)
	switch {
	case _c == 1:
		s := coretypes.ExtractString(_args, 0)
		_res := url.QueryEscape(s)
		return coretypes.MakeString(_res)

	default:
		PanicArity(_c)
	}
	return NIL
}

var __query_unescape__P ProcFn = __query_unescape_
var query_unescape_ Proc = Proc{Fn: __query_unescape__P, Name: "query_unescape_", Package: "std/url"}

func __query_unescape_(_args []coretypes.Object) coretypes.Object {
	_c := len(_args)
	switch {
	case _c == 1:
		s := coretypes.ExtractString(_args, 0)
		_res := queryUnescape(s)
		return coretypes.MakeString(_res)

	default:
		PanicArity(_c)
	}
	return NIL
}

func Init() {

	InternsOrThunks()
}

var urlNamespace = GLOBAL_ENV.EnsureSymbolIsLib(coretypes.MakeSymbol(STRINGS.Intern, "joker.url"))

func init() {
	urlNamespace.Lazy = Init
}
