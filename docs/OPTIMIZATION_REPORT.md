...
2. **String and sequence throughput**
   ...
   - Typed IR v2 now runs automatically for eligible primitive/string loops unless disabled with `JOKER_IR_TYPED=off`, using cached IR analysis and a tagged value stack. It supports constant ASCII and generic string `nth`, internal builder slots, and experimental string-int map values behind `JOKER_IR_TYPED_MAP=1`; probes reduce allocations in string append, char-compare, k-nucleotide, and reverse-complement workloads.
