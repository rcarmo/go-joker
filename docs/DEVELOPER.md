...
Whether built-in (as described below) or separately deployed via source files written in Joker (as described in [Organizing Libraries (Namespaces)](docs/LIBRARIES.md)), developers should be aware of the progression of any given namespace.
...
The set of _core_ namespaces is hardcoded (in any given Joker executable) in `joker.core/*core-namespaces*`; this list is built automatically from information generated by `core/gen/codegen/gen_code.go`.
...
This line builds and runs `core/gen/codegen/gen_code.go`, which finds, in the `CoreSourceFiles` array defined near the top of `core/gen/codegen/gen_code.go`, a list of files (in `core/data/`) to be processed.
...
The list of such dynamic variables is kept in `core/gen/codegen/gen_code.go`, and is currently:
...
* `core/gen/codegen/gen_code.go` imports `std/string`, so the initialization code that adds the namespace is run
...
* A new `core/gen/codegen/gen_code.go` program replaces the old `gen_data.go` program. It generates `a_*code.go` files that mostly define static variables representing the structures resulting when loading `core/data/core.joke` and the like; they are compiled only when building Joker itself. It also does the work that `gen_data.go` used to do for `core/data/linter_*.joke` files. The former root `core/a_data.go` namespace source-list payload is now generated as the data-only manifest in `core/generated` and installed by root `core`.
...
* A new `core/gen/gengo` package is used solely by `gen_code` and implements the details of compiling Go variables into (mostly) static Go code.
...
When built via e.g. `go build -tags go_spew ./cmd/joker`, the private `joker.core/go-spew` function is enabled. (Otherwise it does nothing and returns `false`.)
