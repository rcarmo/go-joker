...
(defn render-sankey [g]
  (let [links (take top-n (sort-by (fn [e] (- (edge-value e))) (:links g)))
        top-node-names (set (map node-name (take top-n (sort-by (fn [n] (- (node-value n))) (:nodes g)))))
        used (if (seq links) (reduce conj (used-node-names links) top-node-names) (set (map node-name (:nodes g))))
        nodes (filter #(contains? used (node-name %)) (:nodes g)) raw-depth-map (depths nodes links) depth-ranks (zipmap (sort (set (vals raw-depth-map))) (range)) depth-map (into {} (map (fn [[k v]] [k (get depth-ranks v 0)]) raw-depth-map)) width 1400 height (max 700 (+ 120 (* 18 (count nodes)))) top 70 bottom 30 left 30 right 260 node-w 6 max-depth (max 1 (reduce max 0 (vals depth-map))) max-edge (max 1 (reduce max 1 (map edge-value links))) max-node (max 1 (reduce max 1 (map node-value nodes))) by-depth (group-by #(get depth-map (node-name %) 0) nodes)
