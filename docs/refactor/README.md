...
Current focused cleanup metrics (2026-05-21): root `core/*.go` is 5 files with one consolidated root test file; `core/types` is 29 files and owns the canonical object/type/protocol/value contracts plus predicates, extractors, string object wrappers, default number equality, runtime hooks, and generated/assertion replacements. `core/types/collections` is 22 files and owns concrete vectors/maps/sets/lists/seqs/chunks plus collection builders. `core/types/string` is 21 files and owns pure string mechanics. `core/runtime` is 41 files and owns runtime state/wrappers/process helpers.
...
- [x] Leave core-specific name derivation co-located with core call mechanics in `core/runtime_kernel.go`.
...
- [x] Add `make std-contract-check` for focused std native-boundary checks (`http`, `io`, `strconv`, `time`, `markdown`, `os`, `system`, `runtime`, `imaging`, `term`, `pdf`, `svg`, `random`, `bolt`, `url`, `git`, `log`, `csv`, `json`, `filepath`, `crypto`, `math`, `string`, `uuid`); see `std-native-boundary.md`.
