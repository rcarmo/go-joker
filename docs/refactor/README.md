...
| `core/reader` | `reader.go`, `read.go`, tagged literals | Reserved package exists as the extraction target; current production reader/expression construction call sites route through `ReaderConstructionAdapter`; leaf helpers such as line rune reading, character classification, and raw buffer/buffered/IO reader mechanics have moved. Move parser/object implementation only after object/tagged-literal/evaluator dependencies are explicit and acyclic. |
