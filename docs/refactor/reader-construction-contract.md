...
3. Move pure lexical/token helpers first if they have no object dependency. **Started: `core/reader` owns character classes; whitespace/comment/line scanning decisions/runs; identifier rune classification, token scanning, validation, keyword/standalone-slash/literal classification, and validation-issue enumeration; initial-token, top-level read-form, and number-token classification; delimiter/comment/top-level-trivia/terminating macro classification; dispatch classification/format-prefix selection, tagged-literal format-prefix and missing-reader mode decisions, and named-character classification; symbolic-value lookup; string/regex/comment/delimiter scanning and invalid-regex mode classification; unicode/octal escape scanning, fixed-width parsing, and string-escape decoding; delimited-form, conditional read-error/suppression decisions, and conditional/unquote/namespaced-map start/prefix/splice helpers; syntax-quote auto-gensym name helpers; pending-form popping; arg-index gap filling/ordering; rune-window history; line rune reading; and raw file/buffer/buffered/IO wrappers.**
