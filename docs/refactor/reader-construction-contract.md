...
3. Move pure lexical/token helpers first if they have no object dependency. **Started: `core/reader` owns character classes; whitespace/comment/line scanning decisions; identifier rune classification, token scanning, validation, and validation-issue enumeration; initial-token and number-token classification; delimiter/comment/terminating macro classification; dispatch and named-character classification; symbolic-value lookup; string/regex/comment/delimiter scanning; unicode/octal escape scanning and fixed-width parsing; delimited-form helpers; pending-form popping; arg-index gap filling; line rune reading; and raw file/buffer/buffered/IO wrappers.**
