...
These remaining files currently stay in package `core` because their generated values refer to many core runtime types and initialization hooks.
...
The guards verify that generated core outputs remain clearly marked, that the generated file set matches `tests/generated_files.txt`, that generator directories still exist, and that the generated source manifest remains in sync with `core/data`. `make docs-check` runs these guards, so generated-file hygiene is part of the standard validation path.
