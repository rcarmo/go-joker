...
- Records, protocols, hierarchy, and namespaces: record and hierarchy behavior is coalesced into `protocol.go`, while namespace behavior is coalesced into `environment.go`; these use moved collection types but remain root-owned runtime/object glue, and record object arity/error behavior goes through `core/types` runtime hooks rather than direct root helpers.
