...
4. Update diagnostics/export/profile/WASM eligibility to read the model. **Started: diagnostics, exported bytecode/slot accessors, pure/imported WASM eligibility, and basic WASM lowering now read the neutral model; profile and WASM memory helper paths remain.**
