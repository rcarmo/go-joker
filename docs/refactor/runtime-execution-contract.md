...
4. Introduce a small runtime execution adapter in root `core`. **Started: `RuntimeExecutionAdapter` now codifies root-owned error creation/throwing, capture-slot installation, nested-call argument/capture preparation, typed env-capture installation, `FnExpr`/slot capture construction, executable metadata reads (slots, bytecode, constants, nested `FnExpr`s, captures), escape/analysis state, call dispatch, typed/boxed execution failure flags, native-helper state, and mem-nth fallback state; boxed and typed executors use it for `irThrow`, capture prefill, `irMakeFn`, nested-call call-slot preparation, typed capture installation/clearing, bytecode/constant access, normal and synthetic-call-expression dispatch, native helper dispatch, mem-nth retry gating, and failure gating. `runtime_execution_boundary_guard_test.go` now rejects direct `prog.*` reach-through, direct callable/runtime dispatch, or ad-hoc adapter construction in executor files.**
...
- Executors and escape analysis: intentionally root-bound pending object/frame contracts becoming narrow enough for real package moves. Boxed, typed, inline typed, and nanbox typed executor files now use `runtimeExec` for executable-envelope state and callable dispatch, but they remain in root `core` because they still depend on `Object`, `irValue`, frame-stack result handling, and object conversion internals.
